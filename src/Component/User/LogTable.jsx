...
const LogTable = ({ ExerciseLog, reload, setReload }) => {
  ...
  return (
    ...
    <div className="mb-5 mt-4 2xl:ml-10 2xl:mr-10 mx-5 border border-dark-blue rounded-3xl pb-10"> {/*container mx-auto max-w-screen-lg*/}
      <table className="flex flex-col bg-dark-blue text-white pt-3 2xl:pt-5 rounded-t-3xl">
        <thead className="">
          
          
          <tr className="flex justify-between items-center py-4">
            <th className="2xl:pl-2 w-2/12 2xl:w-2/12 text-center font-bold uppercase text-white">Activity</th>
            <th className="2xl:pl-2 w-1/12 2xl:w-1/12 text-center font-bold uppercase text-white">Date</th>
            <th className="2xl:pl-0 w-1/12 2xl:w-2/12 text-center font-bold uppercase text-white">Start-Time</th>
            <th className="2xl:pl-1 w-1/12 2xl:w-1/12 text-center font-bold uppercase text-white">Duration</th>
            <th className="2xl:pr-8 w-1/12 pl-4 2xl:w-2/12 text-center font-bold uppercase text-white">Calories</th>
            <th className="2xl:pr-32 w-2/12 pr-4 2xl:w-1/12 text-center font-bold uppercase text-white">Edit</th>
          </tr>
        </thead>
        ...
        <tbody>
          ...
          {currentItems.map((Log, index) => (
            <tr key={Log._id} className={index % 2 === 0 ? "bg-sea-blue" : "bg-white"}>
              <td className="2xl:py-3 w-2/12 2xl:w-2/12 2xl:pl-5 text-center text-black">{revertName(Log.exerciseName)}</td>
              <td className="2xl:py-3 w-1/12 2xl:w-1/12 2xl:pl-10 text-center text-black">{Log.dateTime.split("T")[0]}</td>
              <td className="2xl:py-3 w-1/12 2xl:w-2/12 2xl:pl-10 text-center text-black">{Log.startTime}</td>
              <td className="2xl:py-3 w-1/12 2xl:w-1/12 2xl:pl-24 text-center text-black">{Log.duration}</td>
              <td className="2xl:py-3 w-1/12 2xl:w-1/12 2xl:pl-20 text-center text-black">{Log.calories}</td>

      
      {/*ปุ่ม VIEW DETAIL*/} 
              <td className="2xl:py-3 w-1/12 2xl:w-2/12 text-black whitespace-nowrap">
            
                <button className="btn btn-xs btn-warning border-black 2xl:px-5 2xl:mx-10 2xl:btn 2xl:btn-warning 2xl:border-black" onClick={()=>document.getElementById('my_modal_5').showModal()}>View Details</button>
     ...
     {/*view only Modal*/}
                      ...
                      <h2 className="ml-3 pt-5 text-xl">Activity :
                        <span className="text-xl ml-10 text-white bg-sea-blue border border-slate-300 rounded-lg px-40 py-1"></span>
                      </h2>
                      
                      <h2 className="ml-3 pt-5 text-xl">Date :
                        <span className="text-xl ml-16 text-white bg-sea-blue border border-slate-300 rounded-lg px-40 py-1"></span>
                      </h2>
                    
                      <h2 className="ml-3 pt-5 text-xl">Start-Time :
                        <span className="text-xl ml-3 text-white bg-sea-blue border border-slate-300 rounded-lg px-40 py-1"></span>
                      </h2>
                     
                      <h2 className="ml-3 pt-5 text-xl"> Duration :
                        <span className="text-xl ml-7 text-white bg-sea-blue border border-slate-300 rounded-lg px-40 py-1"></span>
                      </h2>
                    
                      <h2 className="ml-3 pt-5 text-xl"> Calories :
                        <span className="text-xl ml-9 text-white bg-sea-blue border border-slate-300 rounded-lg px-40 py-1"></span>
                      </h2>
          ...
          {/*Edit Modal*/}     
              ...
              {/*Upload image*/}
                             ...
                             <ul>
                              <li className="text-start ml-5">
                                ...
                                <select className="select select-bordered select-sm w-2/3 max-w-xs ml-8 mt-5">
                                  <option disabled selected>Choose new activities</option>
                        ...
                        <td>
                          <button className="btn btn-active hover:btn-error hover:text-white" onClick={() => DeleteHandler(Log._id)}>Delete</button>
                        </td>
                      </div>                                  
                    </div>
                  </dialog>
    ...
    {/*Paginate*/}
      <ReactPaginate
        className={"flex flex-row justify-center gap-8 mx-10 rounded-full border bg-neutral-200 border-neutral-200 py-1"}
        breakLabel="..."
        nextLabel="NEXT >> "
        ...
        pageClassName={'p-2 bg-white rounded-full cursor-pointer'}
        activeClassName={'bg-sky-950 text-white hover:bg-amber-400 hover:text-black'}
